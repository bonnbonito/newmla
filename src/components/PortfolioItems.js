...
const PortfolioItems = () => {
    ...
    const onClickFilter = (e) => {
        e.preventDefault();        
        return (
            setFilter(parseInt(e.target.getAttribute('data-filter')))
        )
    }
        ...
        <div className="container">
            <div className="row">
                <div className="col-md-12">
                    <ul> 
                        <li><a href="/" onClick={onClickFilter} data-filter="0" className={filter === 0 ? 'selected' : ''}>All</a></li>                       
                        {data.allWordpressWpService.edges.map( ({node}) =>  {
                            ...
                            <li key={node.id}>
                                ...
                                className={filter === node.wordpress_id ? 'selected' : ''}
                                >{node.name}
                                </a>
                            </li>
                            )
        ...
        <div className="container">
            <div className="row">  
                {data.allWordpressWpPortfolio.edges.map( ({node}) =>  {
                    ...
                    return (                   
                    <div className={`col-md-4 text-center ${(filter === 0 || node.service.indexOf(filter) !== -1) ? '' : 'hide'}`} style={{marginBottom: `40px`}} key={node.id}>
                        <Link to={node.link}>
                            <div style={{
                                width: `330px`,
                                height: `206px`,
